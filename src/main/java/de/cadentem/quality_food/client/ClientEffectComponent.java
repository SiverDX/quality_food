...
public class ClientEffectComponent implements ClientTooltipComponent {
    ...
    public ClientEffectComponent(final EffectComponent component) {
        MobEffectInstance instance = component.possibleEffect().getFirst();
        ...
        if (instance.getDuration() > 20) {
            effectTooltip = Component.translatable("potion.withDuration", effectTooltip, MobEffectUtil.formatDuration(instance, 1));
        }

        ChatFormatting formatting = instance.getEffect().getCategory().getTooltipFormatting();

        this.text = Component.translatable("potion.withProbability", effectTooltip, FORMAT.format(component.possibleEffect().getSecond() * 100) + "%").withStyle(formatting);
